...
class CmdService {
  ...
  constructor() {
    if (process.platform !== "darwin") {
      ...
      if (dev) {
        this.JavaHome = path.join(
          path.dirname(app.getPath("exe")),
          "tools/jbr"
        );
        this.SdkHome = path.join(
          path.dirname(app.getPath("exe")),
          "tools/toolchains"
        );
      } else {
      ...
      this.JavaHome =
        path.dirname(app.getPath("exe")) + "/../tools/jbr/Contents/Home";
  ...
  exeCmd(cmd, opt = {}) {
    return new Promise((resolve, reject) => {
      console.log(
        `[HDC执行命令] ${cmd.substring(0, 150)}${cmd.length > 150 ? "..." : ""}`
      );
  ...
  async getUdid(device = "127.0.0.1:5557") {
    ...
    try {
      let result = await this.exeCmd(
        `${this.hdc} ${deviceT} shell bm get --udid`
      );
  ...
  async sendFile(
    ...
    filePath = "entry-default-unsigned.hap"
  ) {
    let deviceT = "";
    if (device) {
      deviceT = "-t " + device;
    ...
    await this.exeCmd(
      `${this.hdc} ${deviceT} shell mkdir -p data/local/tmp/hap`
    );
    let result = await this.exeCmd(
      `${this.hdc} ${deviceT} file send "${filePath}" data/local/tmp/hap/`
    );

    if (result.indexOf("finish") > -1) return true;
    else throw Error("安装失败: " + result);
  }
  async installHap(device = "127.0.0.1:5557") {
    let deviceT = "";
    if (device) deviceT = "-t " + device;
    let result = await this.exeCmd(
      `${this.hdc} ${deviceT} shell bm install -p data/local/tmp/hap/`
    );
    console.log("installHap", result);
    if (result.indexOf("successfully") > -1) return true;
    else throw Error("安装失败: " + result);
  }
  async loadModuleJson(filePath) {
    ...
    let moduleInfo = {};
    if (
      zip.getEntries().some((entry) => entry.entryName === fileNameToExtract)
    ) {
      // 读取指定文件内容
  ...
  async createCsr(
    ...
    if (fs.existsSync(csrpath)) return csrpath;
  ...
  async buildApp(
    workdir = "C://Users//Administrator//AppData//Roaming//auto-publish-harmonyos//codetestgo/"
  ) {
    try {
